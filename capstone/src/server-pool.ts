...
export function useServerPool(config: ServerPoolConfig): Operation<ServerPool> {
  return resource<ServerPool>(function* (provide) {
     ...
     *
     * Uses a two-phase construction pattern to avoid `null as any`:
     * 1. Create pending info with known values
     * 2. Build complete ServerInfo once the server handle is available
     */
    function* doSpawnServer(
      hostname: string,
      port: number,
    ): Operation<ServerInfo> {
      // Phase 1: Create pending info with known values (no unsafe casts!)
      const pendingInfo: PendingServerInfo = {
        hostname,
        port,
        startedAt: new Date(),
      };
      ...
      // We'll populate this once the server is ready
      ...
      const task = scope.run(function* (): Operation<void> {
        try {
          ...
          // Phase 2: Build complete ServerInfo with all values populated
          // (task reference will be added after this scope.run() returns)
          const info: Omit<ServerInfo, "task"> = {
            ...pendingInfo,
            app: handle.app,
            server: handle.server,
          };

          // Store in map and resolve the ready promise
          // We'll add the task reference below
          serverInfo = info as ServerInfo;
          servers.set(hostname, serverInfo);

          // Emit started event
          emitEvent({ type: "started", hostname, port });

          // Signal that we're ready (task will be added momentarily)
      ...
      // Add task reference to the server info
      if (serverInfo) {
        serverInfo.task = task;
      }
      serverTasks.set(hostname, task);

      // Wait for the server to be ready and return
      // Use until() to adapt the already-created Promise into an Operation
      // (call() is for invoking functions that return promises; until() is for
      // waiting on promises that already exist - like waiting at a pickup counter
      // for an order that's already cooking, vs placing a new order)
      return yield* until(readyPromise);
    }
    ...
    async function getOrCreate(hostname: string): Promise<ServerInfo> {
      ...
      if (servers.size >= maxServers) {
        const error = new Error(
          `Maximum number of servers (${maxServers}) reached`,
        );
